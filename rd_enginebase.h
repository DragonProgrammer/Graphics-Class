...
class RenderEngine
{
	public:
		  ...
		  /**********************   General functions  *******************************/
virtual int rd_display(const string & name, const string & type,  const string & mode);
...
/*
 ...
 virtual int rd_hyperboloid(const float start[3], const float end[3], float thetamax); 

 virtual int rd_paraboloid(float rmax, float zmin, float zmax, float thetamax);
 virtual int rd_sphere(float radius, float zmin, float zmax, float thetamax);
 virtual int rd_sqsphere(float radius, float north, float east, float zmin, float zmax, float thetamax); 
...
/** virtual int rd_opacity(float opacity);
 ...
 virtual int rd_cone_light(const float pos[3], const float at[3], float theta_min, float theta_max, const float color[], float intensity);

 virtual int rd_point_light(const float pos[3], const float color[], float intensity);

 virtual int rd_far_light  (const float dir[3], const float color[], float intensity);
